...
```
P0 选题(主会话) → P1 文案(主会话) → P2a 搜索(子代理) → P2b 素材(子代理) → P3 视频(子代理) → P4 交付(主会话)
...
**P2 素材阶段 v13.5 重构(搜索独立化 + URL优先级推荐)**
...
// === P2a:搜索阶段(必须先完成)===
{
  "task": "你是视频工厂P2a搜索阶段执行者。工作目录:{outputDir}\n\n⚠️ 核心原则:必须区分视频素材和图片素材,使用各自对应的搜索站点。禁止混用。\n\n## 【第一步:读取config.json】\n读取 {outputDir}/config.json,获取所有场景:\n- 视频场景: assetType=\"video\" 的场景(共V个)\n- 图片场景: assetType=\"image\" 的场景(共I个)\n\n## 【第二步:搜索视频素材】\n对每个视频场景,用 Tavily 搜索（优先）:\n- 命令:node D:/workspace/scripts/tavily_search.cjs --query=\"场景关键词 演示\" --max_results=5\n- Tavily无结果时用 online-search:node prosearch.cjs --keyword=\"场景关键词\" --cnt=5\n- 路径:D:/Program Files/QClaw/resources/openclaw/config/skills/online-search/scripts/prosearch.cjs\n\n🔴 **视频素材搜索站点**(只搜这些,禁止搜新闻网站):\n- 🎬 视频素材站(有直链可下载): vjshi.com | aigei.com | xinpianchang.com | shipin520.com\n- 📺 视频平台(可用yt-dlp下载): bilibili.com | douyin.com | haokan.baidu.com\n- 🌍 海外免费视频(需Referer): pexels.com | pixabay.com\n- ❌ 禁止搜索: sohu.com | 163.com | sina.com.cn | zhihu.com | qq.com | toutiao.com | thepaper.cn (这些是新闻/文章站,yt-dlp无法下载其嵌入视频)\n\n从搜索结果中提取URL时,只取上述视频站点的URL。如果搜索结果没有视频站点URL,则:\n1. 换关键词重搜(如 \"site:pexels.com 关键词\")\n2. 用Pexels API搜索\n3. 仍无结果 → 记录该场景搜索失败,不编造URL\n\n## 【第三步:搜索图片素材】\n对每个图片场景,用 Tavily 搜索（优先）:\n- 命令:node D:/workspace/scripts/tavily_search.cjs --query=\"场景关键词 新闻 报道 截图\" --max_results=5\n- Tavily无结果时用 online-search:node prosearch.cjs --keyword=\"场景关键词 截图\" --cnt=5\n\n📷 **图片素材搜索站点**:\n- 新闻/媒体图片: 163.com | qq.com | sohu.com | sina.com.cn | thepaper.cn\n- 产品官网截图: 用Tavily搜索产品页面URL→HTTP抓取图片\n- 社交媒体: zhihu.com | xiaohongshu.com\n- AI工具界面: 打开对应AI工具官网截图\n\n## 【第四步:写入搜索结果】\n写入 {outputDir}/assets/video_search_results.json,格式:\n{\"scenes\": [{\"sceneId\": 1, \"assetType\": \"video\", \"urls\": [\"url1\", \"url2\"], \"source\": \"域名\"}]}\n\n## 【第五步:验证】\n- 视频场景:URL必须来自视频站点(见上方列表),新闻站URL不计入\n- 图片场景:URL可以是任意网站\n- 有效视频URL数量 >= 视频场景数\n\n6. 全部通过后写 {outputDir}/phase_done_P2a.txt\n工作目录:{outputDir}",
  "mode": "run",
  "runtime": "subagent",
...
// === P2b:下载阶段(必须等P2a完成)===
{
  "task": "你是视频工厂P2b下载阶段执行者。工作目录:{outputDir}\n\n⚠️ 核心原则:素材获取失败=如实报告=不写phase_done_P2.txt。禁止越权兜底。\n\n前提:phase_done_P2a.txt已存在,assets/video_search_results.json已生成。\n\n## 【第一步:读取搜索结果+分类处理】\n读取 {outputDir}/assets/video_search_results.json,获取URL列表。\n按assetType分为视频URL和图片URL,使用不同下载策略。\n\n## 【第二步:下载视频素材(3级下载流程)】\n\n🔴 **核心规则:视频下载不是只有yt-dlp!必须按3级流程依次尝试:**\n\n### Level 1: 直接HTTP下载(素材站CDN直链)\n适用于: vjshi.com | aigei.com | xinpianchang.com | shipin520.com | pexels.com | pixabay.com\n用Node.js https.get下载,必须设置Referer和User-Agent请求头:\n```javascript\nconst https = require('https');\nconst fs = require('fs');\nfunction downloadFile(url, dest) {\n  return new Promise((resolve, reject) => {\n    const file = fs.createWriteStream(dest);\n    https.get(url, {\n      headers: {\n        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; Apple)',\n        'Referer': new URL(url).origin + '/'\n      }\n    }, (res) => {\n      if (res.statusCode === 301 || res.statusCode === 302) {\n        downloadFile(res.headers.location, dest).then(resolve).catch(reject);\n        return;\n      }\n      if (res.statusCode !== 200) { reject(new Error('HTTP ' + res.statusCode)); return; }\n      res.pipe(file);\n      file.on('finish', () => { file.close(); resolve(); });\n    }).on('error', reject);\n  });\n}\n```\n\n### Level 2: yt-dlp下载(视频平台播放页)\n适用于: bilibili.com | douyin.com | haokan.baidu.com\n```bash\npip install yt-dlp -i https://pypi.tuna.tsinghua.edu.cn/simple  # 如未安装\nyt-dlp --no-playlist -f best -o \"{outputDir}/assets/video/scene_NN.%(ext)s\" \"VIDEO_URL\"\n```\n⚠️ yt-dlp只能下载视频平台播放页,不能下载新闻文章页中的嵌入视频!\n\n### Level 3: Tavily搜索图片+HTTP下载(第三手段)
适用于: 产品官网截图、AI工具界面、新闻报道图片
- 用Tavily搜索相关文章页面URL
- 用Python urllib或Node.js https.get抓取页面HTML
- 从HTML中提取<img>标签的图片URL
- 用HTTP下载图片(设置Referer和User-Agent)
- ❌ 禁止打开浏览器/xbrowser（必须静默方式）

### 优先5: Tavily搜索+HTTP下载产品官网截图（必须先尝试）
对每个图片场景：
1. 从config.json的keywords提取截图目标URL
2. 用xbrowser依次打开以下官网：
    - 卲梦AI: https://jimeng.jianying.com
    - 可灵AI: https://kling.kuaishou.com
    - 削明: https://www.jianying.com
3. 截图后保存到assets/image/scene_NN.png
4. 用PIL验证：框屏(width>height)，宽>=800px，>=50KB
5. 成功则跣过其方式

### 优先6: online-search下载真实图片
1. 用prosearch搜索关锠词 + site:163.com / site:qq.com
2. 从搜索结果提取图片URL
3. Node.js https.get下载（设 User-Agent和Referer）
4. 验证：图片>=10KB，框屏(width>height)
5. 成功则记录URL到video_sources.json

### 优先7: PIL生成文字卡片（最后兜底\uff0c<=2【第六步:自验证四步
A:fs.statSync验证存在+size>0+mtime新鲜
B:所有视频MD5去重（MD5相同=同一文件，不能算多个场景）
C:ffprob验证框屏+时长>=3秒
D:水单检查（有水单删了重下）

### ! ! 视频源去重（防止同源片殥复用）
! ! 已知问錶：scene_01和scene_05内容高度重叠，MD5不同但内容重复，违叝真实视频>=2殥内容不同规则。

**视频源去重规则**：
1. 统计唯一源视频数：video_search_results.json中不同源Url算不同源
2. 最小要求：至少>=2个不同源的真实视频
3. 时长重口打查：从同一源截取的多个片殥，时长重口率>30%视为内容重复
    - 重口率=(重口时长/总时长)x100%
    - 例：两个都是0-15s片殥截取出的视频，重口>50%，不合标
4. 如唯一源<2个：必须补充搜索其他源\uff0c不能用同源片殥凑数
5. 记录到video_sources.json：每个场景记录sourceUrl

ttps://www.jianying.com
3. 截图后保存到assets/image/scene_NN.png
4. 用PIL验证：框屏(width>height)，宽>=800px，>=50KB
5. 成功则跣过其方式
...
��有文件】\n每个下载的文件必须验证:\n- 视频:ffprobe验证时长≥3秒+横屏(w>h)+文件≥100KB\n  ffprobe路径:D:/software/ffmpeg-4.4-essentials_build/bin/ffprobe.exe\n- 图片:文件≥10KB+横屏(w>h)\n\n## 【第五步:写video_sources.json】\n每个场景记录:\n{id:N, assetType:\"video\"|\"image\", sourceUrl:\"URL\", localPath:\"assets/video/scene_NN.mp4\", filename:\"xxx.mp4\"}\n❌ localPath不能为空!没有实际文件的场景报告失败。\n\n## 【第六步:运行 validate_p2b.py 自动化验收(强制执行)】\n⚠️ 严禁跳过！必须运行 validate_p2b.py 且 exit code=0 才允许写 phase_done_P2.txt。\n\n```\npython C:/Users/qianq/.qclaw/skills/daily-video-factory/scripts/validate_p2b.py {outputDir}\n```\n\n脚本检查7项门禁（全部通过才exit 0）:\n①assets/image和assets/video目录有实际文件(size>0)\n②所有文件ffprobe/PIL验证横屏(w>h)尺寸≥1280×720\n③video_sources.json的localPath文件真实存在\n④视频场景占比>50%  ⑤MD5去重(无重复凑数)\n⑥真实素材≥60%  ⑦phase_done_P2a.txt存在\n\n⚠️ 禁止子代理自己写 phase_done_P2.txt！\n- exit 0 → 可以写 phase_done_P2.txt\n- exit 1 → 列出错误 → 修复 → 重跑脚本 → 重复直到通过\n\n全部通过后写 {outputDir}/phase_done_P2.txt",
  "mode": "run",
  "runtime": "subagent",
...
```json
{
  "task": "【检查点恢复】检查 phase_done_P3.txt 和 output/ 目录,跳过已生成视频段。你是视频工厂P3视频构建阶段执行者。前提:phase_done_P2.txt 已存在。读取 config.json,**特别注意 config.json.resolution 字段(width/height),所有输出视频的分辨率必须严格匹配**。严格按照 daily-video-factory SKILL.md 和 ACCEPTANCE_RULES.md 执行视频构建、**标题生成与叠加(竖屏视频强制:顶部居中显示Hook/场景标题文字)**、字幕烧录、BGM混合。**⚠️ 竖屏视频必须包含顶部标题(验收铁律 #5.5),缺失标题 = P3 未完成。** 验收通过后写 phase_done_P3.txt 并更新 .task-state.json。工作目录:{outputDir}",
  "mode": "run",
  "runtime": "subagent",
  "label": "P3-video-{date}",
...
async function runPipeline(outputDir, date) {
  ...
  if (whitelistUrls.length < videoSceneCount) {
    throw new Error(`P2a验收失败:有效视频URL=${validUrls.length},需要=${videoSceneCount}`);
  }

  // ========== P2b: 下载阶段(必须等P2a通过)==========
    // 【防错-P2b】先验证config结构,输出场景统计
  ...
  console.log("[P2b] 场景总数:" + scenes.length + " 视频:" + vc + " 图片:" + ic + " | 覆盖率统计以本行数据为准");
  if (vc === 0) throw new Error("config.json中无assetType=video场景,请检查P1输出");
...
P2 完成素材   → **运行 validate_p2b.py 全检**(7项门禁):①assets非空 ②文件size>0 ③ffprobe/PIL验证横屏 ④video_sources.json完整 ⑤视频场景>50% ⑥MD5去重 ⑦真实素材≥60% → exit 0✅通过 → 写 phase_done_P2.txt | exit 1❌不通过 → 返工,不写phase_done,不进P3
