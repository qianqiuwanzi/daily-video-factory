...
description: 自媒体短视频全自动生产线 v11.5（最新：强制先搜索后下载、禁止跳过Tavily搜索环节）。从选题研究、网感文案撰写、TTS配音、真实素材准备、Ken Burns动画视频合成、封面设计到云盘上传的全流程自动化。支持8种稳态Ken Burns动画效果、字幕配音完美同步、真实截图优先规则。适用于每日短视频内容批量生产（抖音、小红书、视频号）。触发场景：每日视频、日更短视频、自媒体内容创作、视频流水线、批量生产视频、产品宣传视频、AI配音视频。
---

# 每日视频工厂 — 全自动短视频生产线 v11.3

从选题到上传，一站式完成每日短视频内容生产。

> **更新（v11.3）**：①**四大源头控制原则**（素材时长、音频优先、字幕规范、生产即验收）②素材下载站点规则 ③文案开头三句改革 ④视频素材占比>50%
...
```json
{
  "task": "【检查点恢复】检查 phase_done_P2.txt 和 assets/ 目录，跳过已完成工作。你是视频工厂P2素材阶段执行者。⚠️ 核心职责：自动完成所有素材收集/制作，严禁依赖用户手动提供。
⚠️ 强制流程（禁止跳过）：
1. 先用 Tavily 搜索视频URL → 获取URL列表
2. 优先尝试搜索结果中的B站/抖音/好看视频等免费平台
3. 搜索结果URL全部失败后 → 才尝试付费站点
4. 全部失败后 → 报告用户，不写phase_done_P2.txt

读取 {outputDir}/config.json，严格按照 daily-video-factory SKILL.md 和 ACCEPTANCE_RULES.md 执行素材准备和TTS生成。验收通过后写 phase_done_P2.txt 并更新 .task-state.json。工作目录：{outputDir}",
  "mode": "run",
  "runtime": "subagent",
...
- ✅ 必须：Tavily 搜索 → 获取URL列表 → 按优先级尝试下载 → 全部失败才换站点
...
| 检查项 | 验收标准 | 如何检查 |
|--------|---------|---------|
| 视频素材真实来源 | 每个视频文件必须有对应的**下载来源URL记录** | 读取 `assets/video_sources.json` 或 `phase_done_P2.txt` 中的 URL 列表 |
| 禁止生成替代 | 每个视频必须是真实的 `.mp4` 下载文件，不能是图片生成的 | 用 ffprobe 检查视频文件是否存在且有真实内容 |
