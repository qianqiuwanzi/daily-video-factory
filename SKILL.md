...
```json
{
  "task": "你是视频工厂P2素材阶段执行者。⚠️ 核心职责：自动完成所有素材收集/制作，严禁依赖用户手动提供。读取 {outputDir}/config.json，完成以下工作：
...
4) 【强制】下载≥2段真实视频素材（对应config.json中assetType=video的场景）
   ⚠️ 视频素材获取优先级：
   a) 优先：从网络下载真实视频（所有途径、所有类型网站：产品官网、YouTube、B站、Pexels、Pixabay等）
   b) 兜底：如下载失败，根据文案内容用AI文生视频功能生成视频素材
   c) 禁止：依赖用户手动提供视频素材
...
### 素材来源优先级（v7.0 更新：自主完成，不依赖用户）

**核心原则**：自动完成所有素材收集/制作，严禁依赖用户手动提供。

#### 图片素材（优先级从高到低）
1. **产品官网截图**（浏览器 CDP 截图，最高优先）
2. **GitHub README/Banner 图片**（raw.githubusercontent.com）
3. **新闻报道/科技媒体配图**（通过搜索获取）
4. **免费图库**（Unsplash、Pexels、Pixabay等）
5. **Canvas 程序化生成**（仅作为**最后兜底**，且必须在验收中标注，占比≤20%）

#### 视频素材（⚠️ v7.0 核心变更）
**获取优先级**：
1. **优先：网络下载真实视频**（所有途径、所有类型网站）
   - 产品官网 Demo 视频
   - YouTube、B站、抖音等视频平台
   - Pexels Videos、Pixabay Videos 等免费视频库
   - 新闻报道/科技媒体视频片段
   - 录屏（浏览器 CDP 录制或本地录制）

2. **兜底：AI 文生视频**（如下载全部失败）
   - 根据场景文案内容，使用 AI 文生视频功能生成视频素材
   - 提示词需与文案语义匹配
   - 生成时长需匹配 TTS 时长
...
### 前置条件
1. Chrome 已启动并开启 CDP：`--remote-debugging-port=28800`
