...
description: 自媒体短视频全自动生产线 v12.1（最新：P2b视频下载具体方法+yt-dlp+文件存在性验收）。从选题研究、网感文案撰写、TTS配音、真实素材准备、Ken Burns动画视频合成、封面设计到云盘上传的全流程自动化。支持8种稳态Ken Burns动画效果、字幕配音完美同步、真实截图优先规则。适用于每日短视频内容批量生产（抖音、小红书、视频号）。触发场景：每日视频、日更短视频、自媒体内容创作、视频流水线、批量生产视频、产品宣传视频、AI配音视频。
---

# 每日视频工厂 — 全自动短视频生产线 v12.1

从选题到上传，一站式完成每日短视频内容生产。

> **更新（v12.1）**：P2b增加yt-dlp视频下载方法+P2验收增加第③+项（视频文件存在性验证），解决"只记URL不下载文件"的越权兜底问题。
> **更新（v12.0）**：①**P2a搜索+P2b下载串联拆分**（搜索独立化，文件交接点强制串联）②**URL白名单验收**（视频场景URL必须来自B站/抖音/好看/VJshi/新片场/爱给/潮点）③**主会话P2a验收**（搜索结果不足则拒绝进入P2b）④**语义分组强制规则**（场景数=语义单元数≠句子数，解决"一句话=一场景"导致素材过多的问题）
...
**约束 2：场景数由总时长和语义决定，不设固定数量（v12.0 强制重构）**
- **原因（v12.0分析）**：一句话一个scene → 16个场景→16个素材需求→素材准备成本高→视频素材比例天然不足。合理分组可以减少素材需求、提高视频素材占比。
- **核心原则**：**场景数 = 内容语义单元数，而非文案句子数**
  - 同一话题的2-3句话合并为1个scene，配1段视频素材
  - 场景数应满足：总时长30-70秒、视频素材占比>50%
- **计算公式**：
  - 总场景数 ≈ Σ(TTS总时长) / 平均场景时长
  - 建议每个scene的TTS时长：视频场景3-10秒，图片场景2-3秒
  - 10-16个场景通常满足30-70秒（取决于每个场景的TTS时长）
- **禁止**：机械地"一句话 = 一个scene"，导致场景数过多、素材成本过高
- **示例**：
  - ❌ 错误：16句话→16个scene→16个素材→视频素材无法覆盖→纯图片混剪
  - ✅ 正确：16句话→8个语义单元→8个scene→8个素材→视频素材占比轻松>50%
- **验收**：P1完成后检查场景数量是否合理（过多场景=未做语义分组=返工）
...
| ③+ | 🔴 **视频文件存在性(v12.1)** | assets/video/*.mp4 实际文件数 ≥ 视频场景数，无文件则该项FAIL | `Get-ChildItem assets/video/*.mp4` 计数 |
