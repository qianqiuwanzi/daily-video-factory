...
description: 自媒体短视频全自动生产线 v11.2（最新：素材下载站点规则统一迁移、国内优先强制规则、Tavily搜索配置、文案开头改革、视频素材>50%、语义关键词匹配）。从选题研究、网感文案撰写、TTS配音、真实素材准备、Ken Burns动画视频合成、封面设计到云盘上传的全流程自动化。支持8种稳态Ken Burns动画效果、字幕配音完美同步、真实截图优先规则。适用于每日短视频内容批量生产（抖音、小红书、视频号）。触发场景：每日视频、日更短视频、自媒体内容创作、视频流水线、批量生产视频、产品宣传视频、AI配音视频。
---

# 每日视频工厂 — 全自动短视频生产线 v11.2

从选题到上传，一站式完成每日短视频内容生产。

> **更新（v11.2）**：①**素材下载站点规则从 TOOLS.md 迁移统一管理**（国内16站+海外3站+Tavily配置+5步搜索优先级）②文案开头三句改革 ③视频素材占比>50% ④文案关键词强化匹配 ⑤视频素材获取优先级
...
```json
{
  "task": "【检查点恢复】检查 phase_done_P2.txt 和 assets/ 目录，跳过已完成工作。你是视频工厂P2素材阶段执行者。⚠️ 核心职责：自动完成所有素材收集/制作，严禁依赖用户手动提供。读取 {outputDir}/config.json，严格按照 daily-video-factory SKILL.md 和 ACCEPTANCE_RULES.md 执行素材准备和TTS生成。验收通过后写 phase_done_P2.txt 并更新 .task-state.json。工作目录：{outputDir}",
  "mode": "run",
  "runtime": "subagent",
...
async function runPipeline(outputDir, date) {
  ...
  // 严格按照 daily-video-factory SKILL.md 和 ACCEPTANCE_RULES.md 执行
...
1. **第一步：搜国内素材站** — 用 Tavily/online-search 搜索「site:vjshi.com AI科技视频」「site:aigei.com 数据流动画」等
2. **第二步：搜国内平台** — 抖音/B站/好看视频搜索关键词获取实拍素材
...
- **Tavily API Key**: `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`
